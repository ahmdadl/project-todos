<div>
    <div class="w-3/4 mx-auto rounded shadow mb-2 bg-gray-300 dark:bg-gray-800">
        <div class='grid grid-cols-1 sm:grid-cols-2 font-semibold'>
            <div x-data="{completed: {{ $todo->completed? 1 : 0 }} }" class='px-2 py-2'>
            <x-jet-button border='' x-ref="check{{$todo->id}}"
                ...
                <span
                    class="{{ $todo->completed ? 'line-through text-green-500' : '' }} inline-block hover:cursor-pointer" x-on:click="$refs['check{{$todo->id}}'] ? $refs['check{{$todo->id}}'].click() : false">
                    {{ $todo->body }}
                </span>
            </div>
            <div class='text-right'>
                <x-jet-button class='h-full -mr-1' rounded='0' bg='blue' wire:click.prevent='edit' icon='fas fa-edit'
                    target='edit, check'>
                </x-jet-button>
